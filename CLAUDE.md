...
用户提供一个文章链接 → 抓取正文 → 判定材料类型 → 提炼海报文案 → 生成 HTML → Chrome headless 截图输出 PNG。
...
通读抓取的正文，对照「材料类型与模块分流」（见 Step 3）中各类型的特征描述判定归属，并明确写出判定结果（类型 + 一句话依据）。判定结果决定后续两步：提炼策略（Step 3）与模板选择（Step 4）。

- 特征明显时直接判定；介于多类之间时，选与文章主体最接近的类型
- 无法判定时默认类型 A，并在报告中说明理由
- 若正文与所有已定义类型都不匹配（如全新体裁），停下来向用户确认，不要硬套

### Step 3: 提炼海报内容

按 Step 2 判定的材料类型，从正文中提取对应模块的文案，将原文压缩为海报语言。
...
当前支持两类材料。各类型只需补充通用原则未覆盖的专属规则；新增类型时在此追加定义。

**类型 A：技术原理类（默认）**
- 特征：围绕单一技术或系统，主体是"它怎么实现的"
- 模块：核心观点 / 技术原理 / 技术价值分析 / 适用场景 / 标签
